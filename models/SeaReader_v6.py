...
# 和seareader v4_5 的区别就是将后面的reasoning 层的lstm换成了三个CNN
class SeaReader_v6(torch.nn.Module):
    """
    ...
    def __init__(self, dataset_h5_path, device):
        super(SeaReader_v6, self).__init__()
        ...
        self.content_reasoning_layer1 =torch.nn.Conv1d(in_channels=hidden_size*4+2,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.content_reasoning_layer2 = torch.nn.Conv1d(in_channels=hidden_size * 4 + 2, out_channels=64, kernel_size=5,stride=1, padding=2)
        self.content_reasoning_layer3 = torch.nn.Conv1d(in_channels=hidden_size , out_channels=hidden_size*2, kernel_size=3,stride=1, padding=1)

        self.question_reasoning_layer1 = torch.nn.Conv1d(in_channels=hidden_size*2+2,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.question_reasoning_layer2 = torch.nn.Conv1d(in_channels=hidden_size*2+2,out_channels=64,kernel_size=5,stride=1,padding=2)
        self.question_reasoning_layer3 = torch.nn.Conv1d(in_channels=hidden_size,out_channels=hidden_size*2,kernel_size=3,stride=1,padding=1)
    ...
    def forward(self, contents, question_ans, logics, questions, answers):
        ...
        # 经过reasoning层
        cur_RnQ_reasoning1 = self.question_reasoning_layer1.forward(gated_cur_RnQ.transpose(1,2))  # size=(16*n,64,100)
        cur_RnQ_reasoning2 = self.question_reasoning_layer2.forward(gated_cur_RnQ.transpose(1, 2))  # size=(16*n,64,100)
        cur_RnQ_reasoning=torch.cat([cur_RnQ_reasoning1,cur_RnQ_reasoning2],dim=1)
        cur_RnQ_reasoning_out=self.question_reasoning_layer3.forward(cur_RnQ_reasoning)# size=(16*n,256,100)

        cur_RmD_reasoning1=self.content_reasoning_layer1.forward(gated_cur_RmD.transpose(1,2))
        cur_RmD_reasoning2=self.content_reasoning_layer2.forward(gated_cur_RmD.transpose(1,2))
        cur_RmD_reasoning=torch.cat([cur_RmD_reasoning1,cur_RmD_reasoning2],dim=1)
        cur_RmD_reasoning_out=self.content_reasoning_layer3.forward(cur_RmD_reasoning)# size=(16*n,256,200)


        # reasoning 层后的pooling 100降维
        RnQ_reasoning_out_max_pooling, _ = torch.max(cur_RnQ_reasoning_out, dim=2)  # size=(16*n,256)
        RmD_reasoning_out_max_pooling, _ = torch.max(cur_RmD_reasoning_out, dim=2)  # size(16*n,256)
