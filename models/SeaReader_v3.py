...
class SeaReader_v3(torch.nn.Module):
    """
    seaReader model for machine comprehension
    Args:
    ...
    def __init__(self, dataset_h5_path, device):
        ...
        self.reasoning_gating_layer = My_gate_layer(hidden_size * 2, gate_choose, dropout_p=dropout_p)

        self.decision_gating_layer = Noise_gate(hidden_size * 4, gate_choose, dropout_p=dropout_p)
        ...
        torch.nn.init.kaiming_normal_(self.decision_layer.weight,a=1)
    ...
    def forward(self, contents, question_ans, logics, questions, answers):
        ...
        viewed_contents = contents.contiguous().view(batch_size * self.use_content_nums,
                                                     max_content_len)  # size=(batch*n, 200)
        viewed_content_vec, viewed_content_mask = self.fixed_embedding.forward(
            viewed_contents)  # size=(batch*n, 200,200)
        viewed_delta_emb = self.delta_embedding.forward(viewed_contents)  # size=(batch*n, 200,200)
        ...
        viewed_RnQ_mask=question_mask.unsqueeze(1).repeat(1,self.use_content_nums,1).view(batch_size*self.use_content_nums,max_question_len)
        viewed_RmD_mask=viewed_content_mask
        ...
        decision_input = torch.nn.functional.dropout(decision_input, self.dropout_p,self.training)  # dropout decision input
