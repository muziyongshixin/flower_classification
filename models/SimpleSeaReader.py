...
class SimpleSeaReader(torch.nn.Module):
    ...
    def __init__(self, dataset_h5_path, device):
        ...
        self.reason_qa=torch.nn.Linear(in_features=hidden_size*2+2,out_features=hidden_size*2,bias=True)
        self.reason_cont=torch.nn.Linear(in_features=hidden_size*4+2,out_features=hidden_size*2,bias=True)


        self.attention_layer_qa=torch.nn.Sequential(
            torch.nn.Linear(in_features=hidden_size * 2+2, out_features=150, bias=True),
            torch.nn.Tanh(),
            torch.nn.Linear(in_features=150,out_features=R,bias=True)
        )
        self.attention_layer_cont=torch.nn.Sequential(
            torch.nn.Linear(in_features=hidden_size *4+2, out_features=150, bias=True),
            torch.nn.Tanh(),
            torch.nn.Linear(in_features=150,out_features=R,bias=True)
        )
    ...
    def forward(self, contents, question_ans, logics, contents_char=None, question_ans_char=None):
        ...
        reason_qa=self.reason_qa.forward(viewed_RnQ)  # size 16*n,100,256
        reason_cont=self.reason_cont.forward(viewed_RmD) # size 16*n,200,256

        attention_qa=self.attention_layer_qa.forward(viewed_RnQ)
        attention_cont=self.attention_layer_cont.forward(viewed_RmD)
        att_qa = torch.nn.functional.softmax(attention_qa, dim=1)  # size=(16*n,100,50)
        att_cont = torch.nn.functional.softmax(attention_cont, dim=1)  # size=(16*n,200,50)

        match_qa=torch.bmm(reason_qa.transpose(1,2),att_qa) # size=(16*n,256,50)
        match_cont=torch.bmm(reason_cont.transpose(1,2),att_cont) # size=(16*n,256,50)

        match_score=match_qa*match_cont # size=(16*n,256,50)
        match_score=torch.sum(match_score,dim=1) # size=(16*n,50)

        viewed_match_score=match_score.view(batch_size,self.use_content_nums,self.R)  # size=(16,n ,50)

        max_pool=torch.max(viewed_match_score,dim=1)[0] # size=(16 ,50)
        mean_pool=torch.mean(viewed_match_score,dim=1)# size=(16 ,50)

        decision_input=torch.cat([max_pool,mean_pool],dim=1) # size=(16,100)
        decision_output=self.decision_layer(decision_input) # size=(16,1)
